...
export const SchemeProvider = ({ children }) => {
  const systemScheme = window.matchMedia('(prefers-color-scheme: dark') ? 'dark' : 'light'

  const [scheme, setScheme] = useState(localStorage.getItem('ncqx-scheme') || systemScheme)
