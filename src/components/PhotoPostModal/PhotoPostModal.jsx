...
const PhotoPostModal = ({ closeModal, hasShotParams, shoudShowDescription, uid }) => {
  const { t } = useI18n()
  ...
  const post = useMemo(() => (data?.photoPosts ? data?.photoPosts[0] : null), [data])

  if (loading) return <LoadingSpinner />
  if (error) return <CatalogContentError />

  return (
    <Modal id={post.id} isOpen={post ? true : false} toggleModal={closeModal}>
      <div
      ...
      {shoudShowDescription && (
        <div className='gallery-modal-description flex flex-col px-6 py-4'>
          {post.location && (
            <div className='shot-location'>
              <p className='mb-2 font-semibold'>{t('postLocation')}</p>
          ...
          {hasShotParams && (
            <div className='shot-params'>
              <p className='mb-2 font-semibold'>{t('parameters')}</p>
              <ul className='px-2'>
                {post.focal_length && (
                  <li>
                ...
                {post.aperture && (
                  <li>
                    ...
                    <span className='clr-primary'> f/{post.aperture}</span>
                  </li>
                )}
                {post.shutter_speed && (
                  <li>
                    <span>{t('shutterSpeed')} :</span>
                    <span className='clr-primary'> {post.shutter_speed}s</span>
                  </li>
                )}
                {post.iso && (
                  <li>
                    <span>{t('iso')} :</span>
                    <span className='clr-primary'> {post.iso}</span>
                  </li>
                )}
              </ul>
            </div>
