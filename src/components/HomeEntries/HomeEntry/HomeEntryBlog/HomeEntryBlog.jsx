          ...
          <Link to={`blog/${post.uid}`} key={index}>
            <div className='flex gap-4'>
              <div className='flex h-[88px] w-[88px]'>
                <img
                  className='h-[88px] w-[88px] min-w-[88px] object-cover object-center'
                  src={`${import.meta.env.VITE_API_URL}${post.thumbnail?.url}`}
                  alt=''
                />
              </div>
              <div className='flex flex-col gap-4'>
                <p className='w-[90%] font-semibold underline'>{post.title}</p>
                <p className='w-[90%] font-light leading-5'>{post.description}</p>
              </div>
            </div>
          </Link>
        ))}
