...
const HomeEntry = ({ id, favorite, latest }) => {
  ...
  return (
    <div className='home-entry'>
      <div className='home-entry-head flex items-center justify-between'>
        <h2 className='py-2'>{t(rowConfig.id)}</h2>
        <Link to={rowConfig.url} className='link-small flex items-center gap-2 pb-2 pl-3 pr-2 pt-2'>
          {t('explore')} <span className='sr-only'>{rowConfig.label}</span>{' '}
      ...
      <div key={rowConfig.id} className='box bordered'>
        ...
        <div className='home-entry-footer'>
          <hr />
          <Link to={rowConfig.url} className='link-small flex items-center justify-end gap-2'>
            {t('explore')} <span className='sr-only'>{rowConfig.label}</span>{' '}
            <Icon height={12} icon='chevron' width={12} />
          </Link>
        </div>
