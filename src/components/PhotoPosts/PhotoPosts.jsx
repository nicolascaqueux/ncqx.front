...
const PhotoPosts = ({ items }) => {
  const { containerRef, handleImgClick, hasShotParams, maxRowHeight, openedPost, rows, spacing } =
    usePhotoPosts({ items })

  const grid = rows.map((row, i) => {
    let containerWidth = containerRef.current?.offsetWidth ?? 1000
    const calculatedRowHeight = (containerWidth - (row.images.length - 1) * spacing) / row.aspectSum

    // Limit height to maxRowHeight
    const rowHeight = Math.min(calculatedRowHeight, maxRowHeight)

    return (
      <div className='row mb-2 flex' key={i}>
          ...
          return (
            <img
              ...
              style={{
                width: `${width}px`,
                height: `${rowHeight}px`,
                marginRight: j !== row.images.length - 1 ? spacing : 0,
              }}
            />
          )
        })}
